...
import { runLocalRunnerOnce } from "./local-runner";
import { WorkflowApiRunnerClient } from "./runner-client";

async function main(): Promise<void> {
  const cliConfig = createCliEngineConfig(process.env);
        ...
        [
          {
            ...
            engine: new GitHubImplementationLocalRunnerEngine({
              client: new GitHubRestClient(githubConfig),
  ...
  const runner = {
    ...
    ownerUserId: process.env.LOCAL_RUNNER_OWNER_USER_ID,
    mode: parseRunnerMode(process.env.LOCAL_RUNNER_MODE),
    ...
    engines: parseList(process.env.LOCAL_RUNNER_ENGINES, [cliConfig.engine]),
    defaultEngine: cliConfig.engine,

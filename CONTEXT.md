...
### Strategy (= Handler 코드 + L4/메타 데이터 hybrid)
Task type / job type 별 차이를 캡슐화하는 정책. **순수 데이터가 아니라 hybrid** (2026-05-27 grilling 결정 — accidental complexity 단순화 A/B/C):

- **task-internal 로직 = Handler 코드** (TypeScript GoF Strategy 구현체):
  - **단일 `EventHandler` interface** 로 통합 (B). 모든 trigger (`task_spawned` / `job_finished` / `external_event`) 가 한 interface.
  - 과거 L1 (Job sequence) / L2 (Outbound mapping) / L3 (Inbound mapping) 의 **로직** 은 handler 코드로 흡수 (C). 조건 분기 (qa_required, 점수 threshold 등) 가 코드로 자연 표현.
- **데이터로 남는 것 = L4 + 메타** (`ai-workflow` repo 의 **`workflows/`** 디렉토리 YAML):
  - **L4 Job spec** — 각 Job 의 I/O JSON schema (Type B, runtime validation 필요) + Type A skill 이름.
  - **메타** — approver role / 산출물 위치 template / **4 axis 값** (아래 Task Pattern 참조) / schema version (I-23).
  - **단순 lookup table** — Outbound 의 status명/comment template, Inbound 의 (외부 transition명 → event type) 매핑.
- **cross-task 오케스트레이션 = Workflow Run Orchestrator 코드** (별도 메커니즘 유지, NOT D):
  - 과거 L5 (Spawn rules) + fan-in (sync gate) + back-edge 는 **handler 가 흡수하지 않고** Orchestrator 의 first-class 책임으로 명시 유지. essential 도메인 복잡도라 가시성 보존.
- **저장 위치**: 별도 repo 아님 — workflow 종류 1 가지 단계에서 YAGNI. 미래 분기 도입 시 repo 분리 재검토.
...
- **SSOT**: `ai-workflow` repo 의 `workflows/` 디렉토리 안 Strategy YAML (또는 같은 디렉토리의 prompt 템플릿 파일).
- **배포**: Workflow App 이 Job spec 만들 때 Strategy YAML 에서 읽어 inline 으로 Runner 에 전달.
- **Version 정책**: Strategy YAML 의 commit hash 가 version (`WorkflowRun.definition_version`). PR review 로 명시 audit.
...
| **Strategy / Type B 통합 Skill** | **`ai-workflow` repo `workflows/` 디렉토리** | 강 (git) | Workflow App memory cache |
...
## Task Pattern — 1 패턴 + 4 axis

**모든 Task 는 단일 패턴의 변이다** (2026-05-27 grilling, 단순화 A — 과거 P1~P4 4 패턴을 통합).
공통 골격: `generate → quality → (revise)* → 사람 승인`. 4 종의 Task 는 아래 **4 axis 값** 만 다르다.

| Task | generate axis | quality axis | revise trigger axis | 사람 승인 source axis |
| --- | --- | --- | --- | --- |
| **Document** (PRD/HLD/LLD/Spec/TC) | AI generate | 점수 (threshold) | "재시도요청" (Jira) | Jira event |
| **Code** | AI gen+test (atomic) | test pass | PR review comment / back-edge | **GitHub event** (PR merge, I-18) |
| **QA** | run_qa | no bugs | new bug / back-edge | Jira event ("QA 승인") |
| **Action-only** (Deploy) | create_ticket | (trivially pass) | (없음) | Jira event ("완료") |

- axis 값은 **handler 코드 + 메타 데이터** 로 표현 (pattern flag 라는 별도 enum 불필요).
- Code 의 LLD/Spec 되돌림 / QA 의 버그 = **back-edge 메커니즘 (I-15)**: AI 가 Bug 티켓 + Jira issue link 자동 생성 → 사람 confirm/수정 → 영향 task "재시도요청" transition → 새 revise Job. 1 bug = 1 revise Job, 같은 task 의 N Job 은 순차 실행 (I-20).
- Code task 의 PR title 에 Jira key 포함 강제 (I-19). Deploy 는 quality/revise 없는 action-only (I-17).

> **검증 결과 (2026-05-27 grilling)**: ✅ 4 종 모두 1 패턴 + 4 axis + 아래 골격 컴포넌트로 표현 가능. 모델 재설계 불필요.

### 골격 컴포넌트 (M0/M1 코드의 구조)

task-internal 로직은 **단일 `EventHandler`** 로 통합 (단순화 B), cross-task 오케스트레이션은 **별도 명시 유지** (NOT D).

| 컴포넌트 | 책임 |
| --- | --- |
| **Task State Machine** | task 단위 lifecycle. registry 에서 task type 별 `EventHandler` lookup → 호출 → return Action 처리. I-6/I-7/I-9/I-20 따름. |
| **Workflow Run Orchestrator** | cross-task: sync gate (I-14, N→1 fan-in), 순방향 fan-out (1→N), 역방향 back-edge (I-15), restart recovery (I-21). **first-class 메커니즘으로 명시 유지** (handler 가 흡수하지 않음). |
| **Outbound Dispatcher** | handler 가 return 한 outbound action 을 외부 시스템 (Jira/GitHub/etc) 에 apply. status명/template 은 메타 데이터. I-19 schema 강제. |
| **Inbound Dispatcher** | 외부 event normalization (Jira / GitHub — I-5 일반화) → (외부 transition명 → event type) lookup table → `EventHandler` 또는 Orchestrator 로 forward. source-agnostic. |

+ **단일 `EventHandler` interface**: `onEvent(event, ctx) → Action[]`. event = `task_spawned | job_finished | external_event`. task type 별 구현체가 registry 에 등록.

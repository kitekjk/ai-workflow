...
- workflow node와 UI primary node는 job 기준으로 둔다. 문서는 job에 연결된 산출물/aggregate로 표시한다.
...
- Claude/Codex runner 또는 사람 개발자가 구현한다.
...
current view는 최신 상태, 최신 artifact, 최신 gate result만 반환한다.
...
GET /agent-jobs/:jobId/logs
```

history/log endpoint는 pagination을 필수로 한다.
...
feedback 저장과 revision 실행을 분리한다. 사용자가 revision 실행을 명시해야 runner job이 생성된다.
...
v1에서 approve/reject는 Jira transition 또는 Jira 상태 refresh와 연결한다.
...
| run tree granularity | stage = node / document = node / job = node | job = canonical node and UI primary node | M1 |
...
- [ ] repository interface와 in-memory 구현을 만든다.
...
- [ ] PRD/HLD/LLD/ADR/Spec 단계별 M2 결정 항목을 확정한다.
- [ ] `document.generate`, `document.evaluate`, `document.revise` job type을 정의한다. Git 저장과 Wiki publish는 generate/revise job 내부 side effect로 처리한다.
- [ ] PRD/HLD/LLD/ADR/Spec prompt contract와 JSON schema를 만든다.
- [ ] PRD approval 이후 downstream routing job을 추가한다.
- [ ] HLD fan-out to LLD, LLD fan-out to Spec workflow job 생성을 구현한다.
- [ ] ADR은 HLD/LLD에서 필요할 때 파생되는 optional document로 모델링한다.
- [ ] document type별 Confluence parent routing을 publisher 설정으로 분리한다.
...
- [ ] PRD/HLD/LLD/Spec approval과 feedback/revision 관련 M3 결정 항목을 확정한다.
- [ ] quality gate result 모델을 document version에 연결한다.
- [ ] approval gate 모델을 Jira transition과 연결한다.
- [ ] Jira comment와 Workflow App feedback을 `feedback_item`으로 저장한다.
- [ ] explicit revision request가 최신 document version을 기준으로 revision job을 생성하게 한다.
- [ ] failed quality gate가 auto rewrite인지 human clarification인지 workflow definition policy로 결정되게 한다.
...
- [ ] runner 권한, workspace, timeout, retry, cancellation 관련 M4 결정 항목을 확정한다.
- [ ] PRD 전용 `scripts/prd-cli-engine.mjs`를 generic document runner bridge로 교체한다.
- [ ] engine별 command/model/sandbox/workdir 설정을 job template로 이동한다.
- [ ] structured JSON schema validation을 추가한다.
- [ ] timeout, retry, cancellation, heartbeat를 runner에 연결한다.
- [ ] workspace preparation과 generated file collection을 구현한다.
- [ ] stdout/stderr/log retention 정책을 추가한다.
...
- [ ] current/history 노출 범위와 dashboard action 관련 M5 결정 항목을 확정한다.
- [ ] workflow run tree API를 추가한다.
- [ ] document current/history API를 안정화한다.
- [ ] job log API를 pagination으로 제공한다.
- [ ] approval/feedback/revision API를 UI action에 맞게 정리한다.
- [ ] `ui-execution-dashboard-demo`를 실제 API에 연결할 수 있는 형태로 재구성한다.
...
- [ ] Dev/Test/Review와 production integration 관련 M6 결정 항목을 확정한다.
- [ ] Jira writeback fields/comment/transition을 구현한다.
- [ ] GitHub branch/PR/status integration을 구현한다.
- [ ] Confluence page parent routing, page version, feedback collection을 강화한다.
- [ ] integration credential 설정과 secret handling을 정리한다.
- [ ] operational metrics, audit events, error alerting을 추가한다.
- [ ] deployment topology와 migration runbook을 문서화한다.
...
2. Generic document pipeline
3. Approval/revision loop
4. Runner hardening
5. Dashboard/API
...
- Confluence feedback 자동 수집은 API 권한과 comment model 확인 전까지 v1 범위에 넣지 않는다.
...
- Dev/Test/Review 자동화 수준은 아직 정책 결정이 필요하다. M6 전까지는 document pipeline의 blocker로 두지 않는다.

...
**불변식 (M0)**: Job atomic — 재시도 = 새 Job 인스턴스. 앱은 문서 *내용*을 절대 보유하지 않음(ref 만, F11 차단). 정책은 데이터(§4), 코드에 type 분기 hardcode 금지(F7 차단).
...
[EventHandler]  job_finished 시 outcome 판정:
   ...
   quality < threshold→ Jira "수정요청"(outbound) → 사람 피드백 대기 (M0: revise 없이 멈춤 허용)
   "승인" transition   → Job(routing) spawn
   routing 완료        → Orchestrator: next_task_candidates 로 다음 Task spawn (M0: 보통 [hld] 후보만 기록하고 run 종료 단계로)
```

- **결정 주체는 앱.** 스킬은 결과 + 후보만 준다. 순차/fan-out 은 앱(M0 은 fan-out 없음).
- **Jira 만 outbound.** 상태/코멘트. git/wiki/PR 없음.
...
```yaml
# _common.yaml (Document 공통 outbound/inbound, base handler 가 읽음)
outbound:
...
- status/transition 문자열은 실제 Jira config 값 — 변경 시 코드 아닌 이 데이터만 수정(F7 차단).
...
| **Scheduler** | Job 스케줄링 (claim/lease — M0 은 단일 러너라 최소) |
| **Runner** | Job claim → 스킬을 AI 엔진으로 호출 → envelope relay. git/wiki/PR 코드 없음 |
...
2. 기획자가 Jira 에서 "승인" → 앱이 routing 실행 → 결과가 Jira + DB 에 일관 기록.
3. 스킬이 만든 wiki/git **ref 가 Task 메타로 저장되고 UI 에서 클릭-이동** 가능.
4. F7(정책=데이터) · F11(앱은 ref 만, 문서내용 무보유) 가 **구조적으로** 재발 불가.
